...
WHERE DATA_HORA_EXECUCAO BETWEEN TO_DATE('2026-01-01', 'YYYY-MM-DD') AND TO_DATE('2026-12-31', 'YYYY-MM-DD');
...
--EXCEPTION WHEN QUANDO NÃO ENCONTRA UM PACIENTE RETORNA UM ERRO
SET SERVEROUTPUT ON;
DECLARE
    v_nome PESSOA.Nome%TYPE;
BEGIN
    SELECT Nome INTO v_nome FROM PESSOA WHERE CPF = '00000000000'; -- CPF Fictício
    DBMS_OUTPUT.PUT_LINE('Nome encontrado: ' || v_nome);
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('ERRO: Paciente com o CPF informado não foi encontrado.');
END;
/

--PARAMETRO DE IN, OUT E IN OUT RECEBE CPF, RETORNA O CRM E INCREMENTA UM CONTADOR
CREATE OR REPLACE PROCEDURE prc_dados_medico (
    p_cpf      IN  MEDICO.CPF%TYPE,
    p_nome     OUT PESSOA.Nome%TYPE,
    p_crm      OUT MEDICO.CRM%TYPE,
...
SET SERVEROUTPUT ON;
DECLARE
    v_nome_medico PESSOA.Nome%TYPE;
    v_crm_medico  MEDICO.CRM%TYPE;
    v_meu_contador NUMBER := 10;
BEGIN
    prc_dados_medico('99988877766', v_nome_medico, v_crm_medico, v_meu_contador);
    DBMS_OUTPUT.PUT_LINE('Nome: ' || v_nome_medico);
    DBMS_OUTPUT.PUT_LINE('CRM: ' || v_crm_medico);
    DBMS_OUTPUT.PUT_LINE('Contador final: ' || v_meu_contador);
END;
/

--CREATE OR REPLACE PACKAGE E CREATE OR REPLACE PACKAGE BODY
-- 17. Especificação do Pacote (o que é público)
CREATE OR REPLACE PACKAGE pkg_gerenciar_exames AS

    PROCEDURE adicionar_exame (
        p_nome IN EXAME.Nome%TYPE,
        p_tipo IN EXAME.Tipo%TYPE
    );

    FUNCTION obter_tipo_exame (
        p_nome IN EXAME.Nome%TYPE
    ) RETURN EXAME.Tipo%TYPE;

END pkg_gerenciar_exames;
/

-- 18. Corpo do Pacote (a implementação)
CREATE OR REPLACE PACKAGE BODY pkg_gerenciar_exames AS
    ...
    BEGIN
        INSERT INTO EXAME (Nome, Tipo) VALUES (p_nome, p_tipo);
        COMMIT;
    END adicionar_exame;

    FUNCTION obter_tipo_exame (
        p_nome IN EXAME.Nome%TYPE
    ) RETURN EXAME.Tipo%TYPE IS
        v_tipo EXAME.Tipo%TYPE;
    BEGIN
        SELECT Tipo INTO v_tipo FROM EXAME WHERE Nome = p_nome;
        RETURN v_tipo;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RETURN 'Tipo não encontrado';
    END obter_tipo_exame;
...
-- Para usar o pacote:
 BEGIN
    pkg_gerenciar_exames.adicionar_exame('Cintilografia Miocárdica', 'Imagem');
 END;
 /

 --CREATE OR REPLACE TRIGGER (COMANDO) uma trigger que registra uma mensagem sempre que atualizam um agendamento
CREATE OR REPLACE TRIGGER trg_cmd_log_agendamento
AFTER UPDATE ON AGENDAMENTO
BEGIN
    DBMS_OUTPUT.PUT_LINE('Atenção: A tabela de agendamentos foi modificada.');
END;
/

--CREATE OR REPLACE TRIGGER (LINHA) criação de uma tabela de auditoria para guardar as mudanças de estados de um agendamento
CREATE TABLE log_auditoria_agendamento (
...
CREATE OR REPLACE TRIGGER trg_linha_audita_status
AFTER UPDATE OF Status ON AGENDAMENTO
FOR EACH ROW 
BEGIN
    INSERT INTO log_auditoria_agendamento (
        agendamento_id,
        status_antigo,
        status_novo,
        data_modificacao
...
-- Para testar, execute um update e depois consulte a tabela de log:
UPDATE AGENDAMENTO SET Status = 'Realizado' WHERE Codigo = 8;
SELECT * FROM log_auditoria_agendamento;
